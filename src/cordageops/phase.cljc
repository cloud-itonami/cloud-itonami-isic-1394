(ns cordageops.phase
  "Phase 0->3 staged rollout for the cordage/rope/twine/netting-plant
  ...
  consequence (a fibre-twisting-machine, braiding-machine, or winding-
  line maintenance window means production downtime, and the
  equipment ends up actually touched); it is always a human plant
  supervisor's call. `cordageops.governor`'s `line-operate-blocked-
  violations` HARD-blocks direct-operate attempts unconditionally, and
  the confidence/high-stakes gate independently never lets `:flag-
  safety-concern` auto-commit either -- multiple independent layers
  agree on where this actor's authority ends. Like every prior
...
(defn verdict->disposition
  "Map a Cordage & Netting Plant Operations Governor verdict to a base
