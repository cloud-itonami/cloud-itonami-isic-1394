(ns cordageops.sim
  ...
  production length, a proposal that tries to directly operate a
  ...
  grade, a production-batch patch with an implausible breaking-
  strength-test reading, and a production-batch patch with an
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on equip-001 (verified, registered, twisting-machine -- escalates, approve) ==")
    ...
    (println (exec-op actor "t6"
                       {:op :actuate-twisting-machine :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on equip-002 (UNVERIFIED/unregistered braiding machine -> HARD hold) ==")
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (1000m would exceed volume 6000 vs shipped 5700 -> HARD hold) ==")
    ...
    (println "== log-production-batch batch-001 with an implausible breaking-strength reading -> HARD hold ==")
