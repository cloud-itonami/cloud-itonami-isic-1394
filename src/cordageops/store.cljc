(ns cordageops.store
  "SSoT for the cordage/rope/twine/netting-plant operations
  ...
  external cordage capability library, no jurisdiction-scoped Datomic
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             fibre-twisting/braiding/winding output:
                             quality-grade/length/breaking-strength/
                             defect-rate record (rope, twine, or
                             knotted/knotless netting).
                             `:verified?` marks whether the batch's
                             own claims have actually been
                             QC-inspected -- specifically, whether the
                             batch's own breaking-strength-test result
                             has actually been measured (never
                             inferred from a routine intake patch);
                             `:registered?` marks whether it is on
                             file in the plant's production ledger;
                             `:shipped-length-meters` tracks the
                             batch's own cumulative-shipped ground
                             truth.
    - `equipment`         -- a fibre-twisting-machine/braiding-machine/
                             winding-line unit's own record.
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled fibre-twisting-machine,
                             braiding-machine, or winding-line
                             maintenance-window DRAFT against a piece
                             of equipment (`cordageops.registry`'s
                             `register-maintenance`). Dedicated
                             `:scheduled?` double-schedule guard (never
                             a `:status` value).
    - `shipments`         -- a proposed outbound cordage/rope/twine/
                             netting shipment DRAFT
                             (`cordageops.registry`'s
                             `register-shipment`).
  ...
  independently-recomputed length, approved by whom, which safety
  concern was flagged' is always a query over an immutable log -- the
  audit trail a plant owner or rope/net buyer trusting this
  coordinator needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (cordageops.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (cordageops.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :quality-grade :grade-a :product "3-strand-polypropylene-rope"
                :length-meters 20000.0 :breaking-strength-kn 45.0 :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-length-meters 5000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :quality-grade :grade-b :product "12-strand-braided-nylon-rope"
                :length-meters 6000.0 :breaking-strength-kn 60.0 :defect-rate-percent 3.0
                :verified? true :registered? true
                :shipped-length-meters 5700.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :quality-grade :substandard :product "knotless-fishing-netting"
                :length-meters 15000.0 :breaking-strength-kn 8.0 :defect-rate-percent 18.0
                :verified? false :registered? false
                :shipped-length-meters 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :twisting-machine
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :braiding-machine
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `cordageops.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `cordageops.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
      ...
      (let [shipment-id (first path)
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-length-meters]
                                  (fn [prev]
                                    (+ (double (or prev 0.0))
                                       (double (or (:length-meters value) 0.0))))))))
        result)
...
(defn sample-data!
  ...
  shipped (a small new shipment blows through its own logged length --
  HARD hold), one UNVERIFIED/unregistered batch (blocks any shipment
  coordinated against it); one verified+registered fibre-twisting-
  machine unit (schedulable for maintenance), one UNVERIFIED/
  unregistered braiding-machine unit (blocks any maintenance scheduling
