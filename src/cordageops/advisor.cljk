(ns cordageops.advisor
  "CordageAdvisor -- the *contained intelligence node* for the
  cordage/rope/twine/netting-plant plant-operations coordination
  actor.

  It normalizes production-batch patches (quality-grade/length/
  breaking-strength/defect-rate), drafts a fibre-twisting-machine,
  braiding-machine, or winding-line maintenance scheduling proposal
  against a piece of equipment, drafts an equipment-safety/quality-
  defect concern flag, and drafts an outbound-cordage-shipment
  coordination proposal against a production batch. CRITICAL: it is a
  smart-but-untrusted advisor. It returns a *proposal* (with a
  rationale + the fields it cited), never a committed record and NEVER
  a real twisting/braiding/winding-line-equipment actuation or freight
  dispatch. Every output is censored downstream by
  `cordageops.governor` before anything touches the SSoT -- see README
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct twisting/
                                 ; braiding/winding-line-equipment-
                                 ; control effect
  ...
  `cordageops.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [kotoba.lang.text :as str]
            [cordageops.registry :as registry]
...
(defn- log-production-batch
  ...
  length, breaking-strength or defect-rate. High confidence, low
...
(defn- schedule-maintenance
  "Draft a fibre-twisting-machine, braiding-machine, or winding-line
  ...
  registered?) in its rationale, but `cordageops.governor` NEVER
...
(defn- flag-safety-concern
  "Draft an equipment-safety/quality-defect concern flag. ALWAYS
  ...
  `cordageops.phase`: no phase ever adds this op to a phase's `:auto`
  set; `cordageops.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound cordage/rope/twine/netting shipment coordination
  proposal against a production batch. The advisor passes through the
  caller's own claimed length -- it does NOT invent one, and
  `cordageops.governor` NEVER trusts it: it independently recomputes
  whether the batch's own cumulative-shipped length plus this claim
  would exceed the batch's own recorded length before any commit is
  possible."
  [db {:keys [subject value]}]
  (let [batch-id (:batch-id value)
        ...
        over-volume? (and b (registry/shipment-volume-exceeded?
                             b (:length-meters value)))]
    {:summary    (str subject " 向け出荷調整提案 ("
                      (:length-meters value) " メートル)"
...
(def ^:private system-prompt
  (str "あなたは撚糸・組紐・巻取工場プラント運用コーディネーターの助言者です。"
       ...
       "撚糸機・組紐機・巻取ラインの設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `cordageops.governor`
