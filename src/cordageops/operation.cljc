(ns cordageops.operation
  "CordageOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (CordageAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Cordage & Netting Plant
  ...
  (:require [langgraph.graph :as g]
            ...
            [cordageops.advisor :as advisor]
            [cordageops.governor :as governor]
            [cordageops.phase :as phase]
...
(defn build
  "Compiles a CordageOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `cordageops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CordageAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Cordage & Netting Plant Operations Governor -- independent censor.
      (g/add-node :govern
