(ns cordageops.governor
  "Cordage & Netting Plant Operations Governor -- the independent
  compliance layer that earns the CordageAdvisor the right to commit.
  ...
  maintenance proposal secretly tries to DIRECTLY OPERATE (rather than
  merely draft-schedule) a fibre-twisting machine, braiding machine,
  or winding line, whether a shipment proposal's own claimed length
  would blow through the batch's own logged production length, or
  when an act stops being a coordination proposal and becomes direct
  twisting/braiding/winding-line-equipment control, so this MUST be a
  separate system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:cordage-netting-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:twister/actuate`
                                       or `:braider/run`) is the
                                       'direct twisting/braiding/
                                       winding-line-equipment control'
    ...
    4. Line-operate blocked        -- for `:schedule-maintenance`, does
                                       ...
                                       Directly operating a fibre-
                                       twisting machine, braiding
                                       machine, or winding line (rather
                                       than merely scheduling a DRAFT
    ...
    5. Equipment not verified/
       registered                  -- for `:schedule-maintenance`,
                                       ...
                                       are both true
                                       (`cordageops.registry/equipment-
                                       ready?`) -- never trust the
                                       advisor's own report about
    ...
    7. Batch not verified/
       registered                  -- for `:coordinate-shipment`,
                                       ...
                                       `:verified?` AND `:registered?`
                                       are both true
                                       (`cordageops.registry/batch-
                                       ready?`) -- never trust the
                                       advisor's own report. Also
    ...
    8. Shipment volume exceeded    -- for `:coordinate-shipment`,
                                       ...
                                       `:shipped-length-meters` plus
                                       the proposal's own claimed
                                       `:length-meters` would exceed
                                       the batch's own recorded
                                       `:length-meters`
                                       (`cordageops.registry/shipment-
                                       volume-exceeded?`) -- ground
                                       truth from the batch's own
                                       permanent fields, never a
                                       self-reported length claim.
    9. Invalid grade               -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:quality-grade` outside the
                                       closed known set
                                       (`cordageops.registry/grade-
                                       valid?`), the batch record is
                                       rejected rather than let a
                                       fabricated grade through.
   10. Invalid breaking-strength   -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:breaking-strength-kn` that is
                                       not a physically plausible
                                       breaking-strength-test reading
                                       (`cordageops.registry/breaking-
                                       strength-valid?`), the batch
                                       record is rejected rather than
                                       let fabricated/instrument-error
                                       data through.
   ...
   12. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       plant supervisor. SOFT: the human
                                       may approve."
  (:require [cordageops.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct twisting/
  braiding/winding-line-equipment-control effect."
  #{:batch/upsert :maintenance/schedule
    :safety-concern/flag :shipment/propose})

(def high-stakes
  "Stakes grave enough to always require a human, even when clean.
  Safety concerns (equipment-safety/quality-defect) are the one op in
  this domain that always demands human eyes regardless of
  confidence."
  #{:coordination/safety-concern})
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct fibre-twisting-machine, braiding-machine, or
  winding-line-equipment control, a fabricated actuation effect) is
  this actor's central scope boundary."
  [proposal]
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は撚糸機・組紐機・巻取ラインの設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- line-operate-blocked-violations
  ...
  directly operate a fibre-twisting machine, braiding machine, or
  winding line -- this actor may only ever propose/schedule a DRAFT
  maintenance window, never operate the line directly. No override,
  ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:direct-operate? (:value proposal))))
    [{:rule :line-operate-blocked
      :detail "撚糸機・組紐機・巻取ライン設備の直接操作提案は恒久的に禁止 -- 保守作業予定(draft)のみ許可"}]))
...
(defn- shipment-volume-exceeded-violations
  ...
  batch's own recorded shipped-to-date length plus the proposal's own
  claimed length would exceed the batch's own recorded
  `:length-meters` -- ground truth from the batch's own permanent
  fields, never a self-reported length claim."
  [{:keys [op]} proposal st]
  (when (= op :coordinate-shipment)
      ...
      (cond
        ...
        [{:rule :shipment-volume-exceeded
          :detail (str batch-id " の記録済み生産量(" (:length-meters b)
                       ...
                       "メートル)+今回申請(" length-meters "メートル)が超過")}]))))
...
(defn- invalid-breaking-strength-violations
  "For `:log-production-batch`, if the patch declares a
  `:breaking-strength-kn` that is not a physically plausible
  breaking-strength-test reading, reject rather than let fabricated/
  instrument-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [bs (:breaking-strength-kn (:value proposal))]
      (when (and (some? bs) (not (registry/breaking-strength-valid? bs)))
        [{:rule :invalid-breaking-strength
          :detail (str bs " kN は物理的に妥当な破断強度試験値の範囲外")}]))))
...
(defn check
  "Censors a CordageAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-breaking-strength-violations request proposal)
