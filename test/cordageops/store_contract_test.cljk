(ns cordageops.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `cordageops.store` ns docstring for why a second (Datomic-
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "equip-002" :concern-type :quality-defect
                                     :severity :high}})
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-volume
  (let [s (seeded)]
    ...
    (is (= 10000.0 (:shipped-length-meters (store/batch s "batch-001")))
