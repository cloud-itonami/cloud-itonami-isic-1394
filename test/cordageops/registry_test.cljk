(ns cordageops.registry-test
  (:require [clojure.test :refer [deftest is]]
            [cordageops.registry :as r]))
...
(deftest small-shipment-within-volume-does-not-exceed
  (is (false? (r/shipment-volume-exceeded?
               {:length-meters 20000.0 :shipped-length-meters 5000.0} 5000.0))))

(deftest shipment-that-pushes-past-volume-exceeds
  (is (true? (r/shipment-volume-exceeded?
              {:length-meters 6000.0 :shipped-length-meters 5700.0} 1000.0))))

(deftest shipment-exactly-at-volume-does-not-exceed
  (is (false? (r/shipment-volume-exceeded?
               {:length-meters 6000.0 :shipped-length-meters 5700.0} 300.0))
...
(deftest missing-volume-is-not-flagged-exceeded
  ...
  (is (false? (r/shipment-volume-exceeded? {:length-meters 800.0} nil))))

;; ----------------------------- grade-valid? -----------------------------

(deftest known-grades-are-valid
  (doseq [g [:grade-a :grade-b :grade-c :substandard :reject]]
    (is (r/grade-valid? g))))
...
;; ----------------------------- breaking-strength-valid? -----------------------------

(deftest typical-breaking-strength-is-valid
  (is (r/breaking-strength-valid? 0.5))
  (is (r/breaking-strength-valid? 45.0))
  (is (r/breaking-strength-valid? 8000.0))
  (is (r/breaking-strength-valid? 20000.0)))

(deftest zero-or-negative-breaking-strength-is-invalid
  (is (not (r/breaking-strength-valid? 0.0)))
  (is (not (r/breaking-strength-valid? -1.0))))

(deftest excessive-breaking-strength-is-invalid
  (is (not (r/breaking-strength-valid? 999999.0)))
  (is (not (r/breaking-strength-valid? 20000.01))))

(deftest non-numeric-or-missing-breaking-strength-is-invalid
  (is (not (r/breaking-strength-valid? nil)))
  (is (not (r/breaking-strength-valid? "45.0"))))
