(ns cordageops.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control fibre-twisting, braiding, or
  winding-line equipment directly... does NOT authorize or execute
  equipment operations') implemented faithfully. The single invariant
  under test:

    CordageAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Cordage & Netting Plant
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-twisting-machine :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest shipment-volume-exceeded-is-held-and-unoverridable
  (testing "a shipment proposal whose length would exceed the batch's own logged volume -> HOLD"
    (let [[db actor] (fresh)
...
(deftest invalid-breaking-strength-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:breaking-strength-kn 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-breaking-strength} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:breaking-strength-kn (store/batch db "batch-001"))) "fabricated breaking-strength never lands in the SSoT")))
